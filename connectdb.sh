...
while [ true ]; do
    ...
    if [[ -d ./databases/$dbname && ! -z $dbname ]]; then
        ...
        select choice in 'Create table' 'List tables' 'insert into table' 'Drop table' 'select  from table' 'list table data' 'update rec' 'delete from rec' 'choose another database' 'go to main menu'; do
            case $REPLY in
            1)
        ...
        select c in "try again" "go back to main menu "; do
            case $REPLY in
            1) . ./connectdb.sh ;;
            2) . ./maindb.sh ;;
            esac
